 ...
 * Uses WeakHashMap to automatically clean up when inventories are garbage collected.
 */
public final class ClickHandlerStore {
  ...
  public static synchronized void attach(Inventory inventory, Map<Integer, MenuSlot.MenuClickHandler> handlers) {
    Objects.requireNonNull(inventory, "inventory must not be null");
    Objects.requireNonNull(handlers, "handlers must not be null");
    HANDLERS.put(inventory, Map.copyOf(handlers));
  }

  public static synchronized MenuSlot.MenuClickHandler find(Inventory inventory, int slot) {
