...
public interface PageControl {
  ...
  default void validate() {
    ...
    if (controlId().isBlank()) throw new IllegalArgumentException("controlId must not be blank");

    Objects.requireNonNull(modeIds(), "modeIds must not be null");
    if (modeIds().isEmpty()) throw new IllegalArgumentException("modeIds must not be empty");

    Objects.requireNonNull(defaultModeId(), "defaultModeId must not be null");
    if (defaultModeId().isBlank()) throw new IllegalArgumentException("defaultModeId must not be blank");
  }
}